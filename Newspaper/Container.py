...
ContainerishSchema = Schema((
    ImageField(
        name='background',
        widget=ImageField._properties['widget'](
            label='Back',
            label_msgid='Newspaper_label_background',
    ...
    BooleanField(
        ...
        widget=BooleanField._properties['widget'](
            label='frame',
            label_msgid='Newspaper_label_frame',
            i18n_domain='Newspaper',
            ),
        ),
    IntegerField(
        ...
        width=IntegerField._properties['widget'](
            ...
            label_msgid='Newspaper_label_backgroundwidth',
            i18n_domain='Newspaper',
            ),
        ),
    IntegerField(
        ...
        width=IntegerField._properties['widget'](
            ...
            label_msgid='Newspaper_label_backgroundheight',
            i18n_domain='Newspaper',
            ),
        ),
),
...
Container_schema = OrderedBaseFolderSchema.copy() + \
    BlockishSchema.copy() + ContainerishSchema.copy()
...
class ContainerJSON(BrowserView):
    ...
    def __call__(self):
        ...
        json_item = self.context.getJSON()
...
#		if fH > useheight:
    ...
    def getWidth(self):
        ...
        for item in items:
            ...
            return pixWidth
        items = self.listFolderContents(contentFilter={"portal_type":"PixLink"})
        for item in items:
            pixWidth=item.getWidth()	
            return pixWidth
    ...
    def callPDFPDTBySameName(self,c,x,y,REQUEST,parent,top,pagenumber,outline):
        ...
        #containercontainer = '/opt/development/newholland/press/products/Newspaper/skins/newspaper_templates/dummy'
        ...
        if self.getFrame():
            theImage = self.getBackground()
            theBlob = theImage.data
            backgroundwidth = self.getBackgroundwidth()
            backgroundheight = self.getBackgroundheight()
            theMask=[0,100,0,100,0,100]
            c.drawImage(ImageReader(StringIO(theBlob)),left-15,top-100,backgroundwidth,backgroundheight,mask=theMask)
        pieces = self.listFolderContents()
        returnx = x
        returny = y
        if outline == True:
            sRed = 1
        ...
        for piece in pieces:
            (piecex,piecey) = piece.callPDFPDTBySameName(c,x,y,REQUEST,self,top,pagenumber,outline)
            if outline == True:
                sRed = 1
                sGreen = 0
                sBlue = 0            
    ...
    def link(self):
        """
        """
        link = "http://www.newhollandpress.com/index/fullarticle/author/"
        items = self.listFolderContents(contentFilter={"portal_type":"TextColumn"})
        for item in items:
    ...
    security.declarePublic('snapshot')
    def snapshot(self):
        """
        """
        width = self.getWidth()
...
#		if fH > height:
        ...
        if numberOfColumns > 0:
            for item in items:
                ...
                #snapresult = item.getSnapshot(width,height,numberOfColumns)
		someitem = snapresult[0]
		#someitem2 = someitem.decode("utf8")
		someitem2 = someitem
		print someitem2
                #snapitem  += snapresult[0]
                snapitem  += someitem2
