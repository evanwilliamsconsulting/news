...
from LetterSchema import LetterSchema
...
class LetterJSON(BrowserView):
	...
	json_item['Letter']=title
	json_item['verbage']=self.context.getLetter()
	pretty = json.dumps(json_item)    
	return pretty 

class Letter(BaseContent, BrowserDefaultMixin):
    """
    """
    security = ClassSecurityInfo()

    implements(interfaces.ILetter,interfaces.IContainer)
    ...
    def returnLine(self,lineid):
	    ...
	    theLetter=self.getWordage()
	    return theLetter[lineStart:lineEnd]
    ...
    def pdf(self,REQUEST): 
		...
		if theChar == '<':
		    ...
		    theCarrotEnd = False
		    theTagOpen = True
		    just = RichColumnar(theLetter,self.getColumnSize())
		    lines = just.getLines()
    ...
    def publish(self,REQUEST):
            ...
            """
	    skin = self.portal_skins.newspaper_templates
	    showTemplate=skin.publishLetter
	    return showTemplate()
