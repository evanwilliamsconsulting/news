...
class PDFPageTemplate(FSObject, Script, PageTemplate):
    ...
    def continuePDF(self,c,x,y,request,parent,pagenumber,outline):
	#script = self.script
	...
	for aLine in parent.listFolderContents():
                theId = aLine.getId()
		result=self.callPDFTemplate(theId,c,x,y,request,parent,pagenumber,outline)
                #x=result[0]
                #y=result[1]
	return (x,y)

    def pdf(self,request,parent,outline):
	return self.pt_render(request,parent,outline)
    ...
    def callTemplate(self,requestedTemplate,request,parent):
	    ...
	    parent = result
	    result = result.callPDTBySameName(all,request,parent,0,0,False)
	    result = str(result)
	    #except:
    ...
    def callPDFTemplate(self,result,c,x,y,request,parent,pagenumber,outline):
	    # Call the web function on the requested template
	    # In the same zopish folder as this template.
	    toplevel = request.PARENTS[0]
	    #result = str(theContent.__dict__)
	    ...
	    result = result.callPDFPDTBySameName(c,x,y,request,parent,toplevel,pagenumber,outline)
	    #except:
	    # 	result = requestedTemplate
            #x=result[0]
            #y=result[1]
    ...
    def renderPageHeader(self,c,x,y,pagenumber,parent):
	    ...
	    if pagenumber == 'pagetwo':
	        number = 2
                ...
                number = 4
            elif pagenumber == 'pagefive':
                number = 5
            elif pagenumber == 'pagesix':
                number = 6
            elif pagenumber == 'pageseven':
                number = 7
	    else:
		number = 8
            textobject = c.beginText()
            textobject.setTextOrigin(x,y+40)
            textobject.setFont('CaslonBold',16)
	    ...
	    #theDateOfPublication = "2014:07:03"
    ...
    def renderMasthead(self,c,x,y,parent):
	    ...
	    #y = y - 100
	    #fptr = open('/root/Windmill.jpg',"r")
	    #theTurkey = ImageReader(fptr)
	    #theMask = [100,255,100,255,100,255]
	    #c.drawImage(theTurkey,x-25,y-35,100,125,mask=theMask)
	    #c.drawImage(theTurkey,x+300,y-400,825,882)
            textobject = c.beginText()
            textobject.setTextOrigin(x-30,y)
	    ...
	    #c.drawImage(thePress,x+615,y-115,width,height)
            textobject.setFont('CaslonBold',88)
	    textobject.setFillColorRGB(0,0,0)
	    textobject.textLine("New Holland Press")
	    textobject.setTextOrigin(x+10,y-20)
            textobject.setFont("CaslonBold",16)
	    textobject.setFillColorRGB(0,0,0)
            textobject.textLine("A Paper Of Discussion")
            textobject.setFont("CaslonBold",16)
	    #textobject.setFillColor(HexColor(0xA7A7E6))
	    textobject.setFillColorRGB(0,0,0)
            textobject.setTextOrigin(x+470,y-20)
            textobject.textLine("www.newhollandpress.com")
	    textobject.setTextOrigin(x+525,y+65)
            textobject.textLine(useThisDate)
	    textobject.setFillColorRGB(0,0,0)
	    #textobject.setFillColor(HexColor(0x22f700))
	    textobject.setTextOrigin(x+55,y+65)
            #textobject.textLine("Happy Halloween!")
            #textobject.setTextOrigin(x+275,y+45)
    ...
    def pt_render(self, request, parent,outline):
        #self._updateFromFS()  # Make sure the template has been loaded.

	#fptr = open (self._filepath,"r")
	fptr = open ("/opt/newholland/press/products/Newspaper/issue.pd","r")
	allLines = fptr.readlines()
	...
	c = canvas.Canvas(output,pagesize=(735,1152),bottomup=1)
	x=35
	y=0
	all = ""
	for aLine in allLines:
		...
		if aLine=='masthead':
			...
			all+=''
        		...
        		pdfmetrics.registerFont(TTFont('Bold','timesbd.ttf'))
        		pdfmetrics.registerFont(TTFont('Italic','timesi.ttf'))
        		pdfmetrics.registerFont(TTFont('Caslon','caslon.ttf'))
        		pdfmetrics.registerFont(TTFont('CaslonItalic','caslonitalic.ttf'))
        		pdfmetrics.registerFont(TTFont('CaslonBold','caslonbold.ttf'))
        		pdfmetrics.registerFont(TTFont('VeraBd','VeraBd.ttf'))
			y = 150
			pagenumber = 1
		        result=self.callPDFTemplate(aLine,c,x,y,request,parent,pagenumber,outline)
                        x+=result[0]
                        y+=result[1]
		elif aLine=='save':
			all+=''
